...
session_start();
  ...
  {
    header('Location: salida_almacenn.php');
    mysqli_close($conection);
  }
  $folio = $_REQUEST['id'];

  $sqlact= mysqli_query($conection,"SELECT * FROM salida_almacen WHERE folio = $folio");
  mysqli_close($conection);
  $result_sqlact = mysqli_num_rows($sqlact);

  if($result_sqlact == 0){
    header('Location: salida_almacene.php');
  }else{
    $option = '';
  ...
  $sqlprod   = "select iv.id, iv.codigo, iv.marca, iv.descripcion, iv.umedida from refacciones iv ORDER BY iv.codigo";
  $queryprod = mysqli_query($conection, $sqlprod);
  $filasprod = mysqli_fetch_all($queryprod, MYSQLI_ASSOC);

  $sqlprodnm = "select iv.id, iv.codigo, iv.marca, iv.descripcion, iv.umedida from refacciones iv ORDER BY iv.descripcion";
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
        ...
        <div class="row">
          ...
          <div class="col-sm-6 d-none d-sm-block">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Salida de Almacen</a></li>
    ...
    <center>
      ...
      /*              
          include "../conexion.php";
          $query_folio = mysqli_query($conection,"SELECT * FROM folios where serie = 'SA'");
          ...
          $query_upfolio = mysqli_query($conection,"UPDATE folios SET folio= folio + 1 where serie = 'SA'");
     ...
     <div class="card card-secondary">
              <div class="card-header">
                <h3 class="card-title">Edita Salida de Almacén</h3>
              </div>
              <div class="card-body">
                 ...
                 <div class="form-group row" >
                   ...
                   <div class="col-sm-12">
                          ...
                          <thead>
                           ...
                           <tbody id="detalle_salidaalm">
                               <!---Contenido Ajax--->
                           </tbody>
                           
                            <tfoot id="detalle_totsalidaalm">
                            <!-- Contenido Ajax -->    
                           </tfoot>
                          </table>
                  ...
                  <div class="form-group row" style="text-align:left;">
                    ...
                    <div class="col-sm-10">
                      <textarea class="form-control" id="inputNotas" name="inputNotas" rows="1"><?php echo $observaciones;?></textarea>
                    </div>
                  </div>
        ...
        if (resultado.value) {
          ...
          /*
           ...
           var norecibo = $('#inputFolio').val();
            var action = 'procesarSalirSalida';
            ...
            $.ajax({
                ...
                success: function(response)
                {
                    ...
                    if(response != 'error')
                    {
                      ...
                      location.href = 'salida_almacen.php';
                       //*location.reload();
...
<script>
   $('#guardar_tipoactividad').click(function(e){
       ...
       var action       = 'AlmacenaEditSalidaAlm';
                    ...
                    success: function(response)
                    {
                      if(response != 'error')
                        ...
                        $mensaje=(info.mensaje);
                         ...
                         .fire({
                          title: "Exito!",
                          text: "SALIDA DE ALMACEN EDITADA CORRECTAMENTE",
                          icon: 'success',
      ...
      <div class="modal-body">
        ...
        <div class="form-group row" hidden>
         ...
         <div class="form-group row" hidden>
           ...
           <div class="col-sm-9">
            <input type="text" class="form-control" id="inputfoliodet" name="inputfoliodet" value="<?php echo $folio; ?>" readonly>
           </div>
        </div> 
...
<script>
   $('#actualizaclientes').click(function(e){
       ...
       var action       = 'AddDetalleEditSalidaalm';
                      ...
                      if(response != 'error')
                        {
                            ...
                            $('#detalle_salidaalm').html(info.detalle);
                            $('#detalle_totsalidaalm').html(info.totales);
     ...
     <script> 
    function del_detalle_cotizacion(id, folio, codigo){
    var action = 'delDetEditSalidaalm';
    var id_det = id;
    var folio_det = folio;
    var c_codigo = codigo;
    ...
    $.ajax({
        ...
        type: "POST",
        async : true,
        data: {action:action, id_det:id_det, folio_det:folio_det, c_codigo:c_codigo},
                      ...
                      if(response != 'error')
                        {
                           ...
                           $('#detalle_salidaalm').html('');
                           $('#detalle_totsalidaalm').html('');
      ...
      <div class="modal-body">
        ...
        <div class="col-md-12">
          <div class="form-group"> 
           ...
           <div class="col-sm-10">
            <select class="form-control" style="width: 100%; text-align: left" id="txt_codigo" name="txt_codigo" readonly>
                <option value="">- Seleccione -</option>
                <?php foreach ($filasprod as $oppd): //llenar las opciones del primer select ?>
                <option value="<?= $oppd['codigo'] ?>"><?= $oppd['codigo']?></option>  
                <?php endforeach; ?>
         ...
         <div class="col-md-12">
          ...
          <div class="form-group row">
           ...
           <div class="col-sm-10">
            <select class="form-control" style="width: 100%; text-align: left" id="txt_descripcion" name="txt_descripcion" readonly>
                <option value="">- Seleccione -</option>
                <?php foreach ($filasprodnm as $opnm): //llenar las opciones del primer select ?>
                <option value="<?= $opnm['descripcion'] ?>"><?= $opnm['descripcion']?></option>  
                <?php endforeach; ?>
        ...
        <div class="col-md-12">
         ...
         <div class="form-group row">
           ...
           <div class="col-sm-10">
            <input type="text" class="form-control" id="txt_marca" name="txt_marca" readonly>
           </div>
        </div>
        ...
        <div class="form-group row">
           ...
           <div class="col-sm-10">
            <select class="form-control" style="width: 100%; text-align: left" id="txt_unidadmedida" name="txt_unidadmedida" readonly>
                <option value="">- Seleccione -</option>
        ...
        <div class="form-group row">
           ...
           <div class="col-sm-10">
            <input type="number" step="any" class="form-control" id="txt_precio" name="txt_precio" readonly>
           </div>
        </div>
...
<script>
   $('#actualiza_paradar').click(function(e){
        ...
        var action       = 'ActualizaMovsalidaalm';
...
<script type="text/javascript">
    ...
    $url = 'factura/salidaalmacen.php?id='+ id;
    window.open($url,"Salida Almacen","left="+x+",top="+y+",height="+alto+",width="+ancho+",scrollbar=si,location=no,resizable=si,menubar=no");
  ...
  <script type="text/javascript">
    $(document).ready(function(){
        ...
        //alert(c_noorden);
        searchForDetalleditSalida(c_folio);
     });
  </script>

 <script>
 function searchForDetalleditSalida(c_folio){
    var action  = 'searchForDetalleditSalida';
    var nofolio = c_folio;
