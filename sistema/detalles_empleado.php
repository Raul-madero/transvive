                ...
                <div class="tab-content">
                  <div class="active tab-pane" id="activity">
                    ...
                    <input type="hidden" class="form-control" id="inputId" name="inputId" value="<?php echo $id; ?>" readonly>  
                      ...
                      <a href="<?php echo ($estatus === '1') ? 'empleados.php?id=' . $noempl : 'empleadosBajas.php?id=' . $noempl; ?>" 
                      type="button" class="btn btn-secondary" id="btn_salir">Volver</a>
                      </div>
                  ...
                  <div class="post clearfix">
                    ...
                    <div class="form-group row" >
                      ...
                      <div class="form-group row">
                        <label for="inputName" class="col-sm-3 col-form-label mt-4" style="text-align: left;">Alta IMSS:</label>
                        <div class="col-sm-3 mt-4" style="text-align:left;">
                            <select class="form-control select2bs4 mt-4" id="inputImss" name="inputImss">
                                <option value="<?php echo $imss; ?>"><?php echo $imss; ?></option>
                            </select>
                        </div>
                        <label for="inputName" class="col-sm-3 col-form-label mt-4" style="text-align: left;">Fecha Alta IMSS:</label>
                        <div class="col-sm-3" style="text-align:left;">
                        <input type="date" class="form-control mt-4" id="inputDatealtaimss" name="inputDatealtaimss"  value="<?php echo $fecha_altaimss; ?>" readonly>
                        </div>
                      </div>
   ...
   $('#guardar_cliente').click(function(e){
       ...
       var action       = 'AlmacenaEditEmpleado';
        ...
        $.ajax({
                    ...
                    type: "POST",
                    async : true,
                    data: {action:action, Id:Id, noempleado:noempleado, name:name, paterno:paterno, materno:materno, cargo:cargo, telefono:telefono, rfccte:rfccte, unidad:unidad, nounidad:nounidad, tipo_lic:tipo_lic, nolicencia:nolicencia, fvencimiento:fvencimiento, supervisor:supervisor, tipocontrato:tipocontrato, fcontrato:fcontrato, vencontrato:vencontrato, imss:imss, salariodia:salariodia, sueldobase:sueldobase, sueldo:sueldo, sueldob2:sueldob2, vdgmv:vdgmv, vdgao:vdgao, sprinter:sprinter, sueldo_auto:sueldo_auto, ssemi:ssemi, deuda:deuda, descuento:descuento, adeudo:adeudo, saldo_adeudo:saldo_adeudo, bonos:bonos, clasif_cat:clasif_cat, bonosc2:bonosc2, bonosemanal:bonosemanal, apoyomes:apoyomes, vales:vales, caja:caja, vacaciones:vacaciones, efectivo:efectivo, descefectivo:descefectivo, tipo_nomina:tipo_nomina, sexo:sexo, fechanac:fechanac, edad:edad, edocivil:edocivil, domicilio:domicilio, estudios:estudios, contactoe:contactoe, elcurp:elcurp, fchaaltaimss:fchaaltaimss, noss:noss, salarioxdia:salarioxdia, sueldoauto:sueldoauto, sdosprinter:sdosprinter, es_recontrata:es_recontrata, recontratable:recontratable, comentarios:comentarios, datebaja:datebaja, datereingreso:datereingreso},
...
<script>
 ...
 function searchForDetalleditPeriodo(c_noemp){
    ...
    $.ajax({
        ...
        type: "POST",
        async : true,
        data: {action:action,noempl:noempl},
...
<script> 
    ...
    $.ajax({
        ...
        type: "POST",
        async : true,
        data: {action:action,id_partida:id_partida,no_emp:no_emp},
