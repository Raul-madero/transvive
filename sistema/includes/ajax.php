...
//Envio encuesta de Calidad
    ...
    if (!empty($_POST['name']) && !empty($_POST['paterno']) && !empty($_POST['materno'])) {
        ...
        $correo        = $_POST['correo'];
        $nombre_emergencia = $_POST['nombreEmergencia'];
        $telefono_emergencia = $_POST['telefonoEmergencia'];
        $parentesco = $_POST['parentesco'];

        // Fechas con valores por defecto
        $datebaja      = !empty($_POST['datebaja']) ?"'" .  $_POST['datebaja'] . "'" : "NULL";
        $datereingreso = !empty($_POST['datereingreso']) ?"'" .  $_POST['datereingreso'] . "'" : "NULL";
        ...
        // Consulta SQL
        $sql_editar_empleado = "UPDATE empleados SET 
            nombres = '$name',
            ...
            fecha_fincontrato = $fincontrato,
            imss = '$imss',
            salario_diario = $salariodia,
            sueldo_base = $sueldobase,
            sueldo_camion = $sueldo,
            sueldo_camioneta = $sueldob2,
            bono_semanal = $bonosemanal,
            sueldo_especialcamion = $vdgmv,
            sueldo_especialcamioneta = $vdgao,
            sueldo_especialsprinter = $sprinter,
            sueldo_especialauto = $sauto,
            sueldo_especialsemi = $ssemi,
            deuda_general = $deuda,
            descuento = $descuento,
            adeudo = $adeudo,
            saldo_adeudo = $saldo_adeudo,
            bono_supervisor = $bono,
            bono_categoria = $bonoc2,
            apoyo_mes = $apoyomes,
            sueldo_adicional = $vales,
            caja_ahorro = $caja,
            vacaciones = $vacaciones,
            efectivo = $efectivo,
            descuento_fiscal = $descfiscal,
            tipo_nomina = '$tipo_nomina',
            sexo = '$sexo',
            date_nacimiento = $fechanac,
            edad = $edad,
            estado_civil = '$edocivil',
            domicilio = '$domicilio',
            estudios = '$estudios',
            contacto_emergencia = '$contactoe',
            curp = '$elcurp',
            fecha_altaimss = $fchaaltaimss,
            numeross = '$noss',
            salarioxdia = $salarioxdia,
            sueldo_coche = $sueldoauto,
            sueldo_sprinter = $sdosprinter,
            recontratable = '$recontratable',
            pqrecontrata = '$es_recontrata',
            comentarios = '$comentarios',
            fecha_baja = $datebaja,
            fecha_reingreso = $datereingreso,
            edit_id = '$usuario',
            correo = '$correo',
        ...
        if (!$query_editar_empleado) {
            echo json_encode(array('error' => mysqli_error($conection)));
            exit;
        }
        ...
        if ($result_detalle > 0) {
            ...
            echo json_encode(array('error' => 'No se actualizó ningún registro'));
        }
...
if ($_POST['action'] == 'EditaAlmacenaViaje') {
    ...
    $sql = "UPDATE registro_viajes 
            SET 
                ...
                id_supervisor = ?, notas = ?, usuario_edit = ?, estatus = ?, justificacion_cambio_unidad = ? 
            WHERE id = ?";

    $stmt = mysqli_prepare($conection, $sql);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, 'sssssssssiissssddssisi',
            ...
            $idsuperv, $notas, $usuario, $estatus, $justificacion,
            $id
        );
        ...
        if($_POST['action'] == 'searchSueldoVueltavalor')
        {
            if(!empty($_POST['op'])){
                ...
                if($tipo_viaje == 'Especial' || $tipo_viaje == 'Especial Turistico') {
                    ...
                    $stmt->execute();
                    $stmt->bind_result($sueldo_vuelta);
                    if($stmt->fetch()) {
                ...
                if($tipo_viaje === 'Semidomiciliadas' && $unidad === 'Camioneta') {
                    ...
                    $stmt->close();
                }elseif ($tipo_viaje === 'Semidomiciliadas' && $unidad === 'Sprinter') {
...
if ($_POST['action'] == 'AddDetallecotizacion') {
    ...
    $impuesto     = floatval($_POST['impuesto']);
    $impuestoisr  = floatval($_POST['impuestoisr']);
    $impuestoieps = floatval($_POST['impuestoieps']);
    $importe      = floatval($_POST['importe']);
    $datoe        = mysqli_real_escape_string($conection, $_POST['datoe']);
    $datoom       = mysqli_real_escape_string($conection, $_POST['datoom']);
    $token        = md5($_SESSION['idUser']);

    // Inserción
    $query_insert = "INSERT INTO detalle_temp_cotizacioncompra 
        (folio, codigo, descripcion, marca, cantidad, precio, impuesto, impuesto_isr, impuesto_ieps, importe, dato_e, dato_om, token) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = mysqli_prepare($conection, $query_insert);
    mysqli_stmt_bind_param($stmt, 'isssddddddsss', $nofolio, $codigo, $descripcion, $marca, $cantidad, $precio, $impuesto, $impuestoisr, $impuestoieps, $importe, $datoe, $datoom, $token);
    ...
    $sql_select = "SELECT * FROM detalle_temp_cotizacioncompra WHERE folio = $nofolio";
    $query_result = mysqli_query($conection, $sql_select);
...
if ($_POST['action'] == 'AddDetalleOrdencompra') {
    ...
    // Verificar si hay errores antes de la consulta
    if (!$nofolio || !$codigo || !$descripcion) {
        echo json_encode(["status" => "error", "message" => "Faltan datos obligatorios"]);
        exit;
    }

    // Insertar en la base de datos
    $query = "INSERT INTO detalle_temp_ordencompra (folio, no_req, codigo, descripcion, umedida, marca, cantidad, precio, impuesto, impuesto_isr, impuesto_ieps, importe, token) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt = mysqli_prepare($conection, $query)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            $query_detalle = mysqli_query($conection, "SELECT * FROM detalle_temp_ordencompra WHERE folio = $nofolio");
            $detalleTablaPe = '';
            $detalleTotalesPe = '';
            ...
            while ($data = mysqli_fetch_assoc($query_detalle)) {
                ...
                $detalleTablaPe .= '<tr>
                                    ...
                                    <td align="center">
                                        <a href="#" class="eliminar" data-id="' . $data['id'] . '"><i class="far fa-trash-alt"></i></a>
                                    </td>
                                  </tr>';
            }
            ...
            echo json_encode(["status" => "error", "message" => "Error al insertar en la BD: " . mysqli_error($conection)]);
        }
        mysqli_stmt_close($stmt);
    } else {
