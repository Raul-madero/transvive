        ...
        if($_POST['action'] == 'ActualizaNominaSemanal')
                {
                    ...
                    $query_insert = mysqli_query($conection,"INSERT INTO dias_alertas (name_empleado, noalertas) SELECT operador, noalertas from alertas where semana = '$semana' and YEAR(fecha) = YEAR(CURDATE()) group by operador ");

                    $query_procesarcf = mysqli_query($conection,"CALL procesa_nomsemanal23('$semana')");
                $result_procesarcf = mysqli_num_rows($query_procesarcf);
...
if($_POST['action'] == 'AlmacenaSolicitudmpreventivo')
{
    if(empty($_POST['fecha']) || empty($_POST['nounidad']) || empty($_POST['operador']) || empty($_POST['solicita']) || empty($_POST['trabajosolic']) )
    {
       echo 'error';
        ...
        $query_insertar_mantto_preventivo = "SELECT COUNT(*) INTO @verifica FROM manverfiica2tenimiento_preventivo WHERE no_orden = $folio;
                            IF @verifica = 0 THEN 
                            SELECT COUNT(*) INTO @ FROM solicitud_mantenimiento WHERE no_orden = $folio;
                            IF @verifica2 = 0 THEN
...
if($_POST['action'] == 'BajaSolicitudPreventivo')
{
        ...
        $usuario     = $_SESSION['idUser'];
            ...
            $query_procesar = mysqli_query($conection,"CALL baja_solicitudprev($idc, $noorden, $usuario)");
            $result_detalle = mysqli_num_rows($query_procesar);
