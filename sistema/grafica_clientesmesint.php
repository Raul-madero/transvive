...
session_start();
  ...
  $sqltot= mysqli_query($conection,"SELECT count(*) as tot_quejas FROM no_conformidades WHERE YEAR(fecha) = YEAR(CURDATE()) and tipo_incidente = 'INTERNO' ");
  ...
  while ($drowt = mysqli_fetch_array($sqlaniotot)){
   ...
   $no_julio[] = $drowt['Jul'];
   $no_agosto[] = $drowt['Ago'];
   $no_septiembre[] = $drowt['Sep'];
   $no_octubre[] = $drowt['Oct'];
   $no_noviembre[] = $drowt['Nov'];
   $no_diciembre[] = $drowt['Dic'];
...
// Create the chart
  ...
  Highcharts.chart('container', {
    ...
    title: {
        text: 'No conformidades por Cliente por Mes (MOTIVO INTERNO)',
        align: 'left'
    },
    subtitle: {
        text:
    ...
    xAxis: {
        categories: ['<?php echo join($name_cliente, "','") ?>'],
        crosshair: true,
        accessibility: {
