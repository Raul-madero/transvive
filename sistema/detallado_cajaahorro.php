...
session_start();
  ...
  $sqlopr   = "select noempleado, CONCAT(nombres, ' ', apellido_paterno, ' ', apellido_materno) as nombre_empleado from empleados ORDER BY noempleado";
  ...
  <div class="content-wrapper">
   <section class="content">
        <div class="container-fluid">
            <h2 class="text-center">Reporte Detallado de Caja de Ahorro</h2>
            <form id="filtroReporte" method="POST" action="reporte_cajaahorro.php" target="_blank">
                <div class="row">
                          ...
                          <div class="col-4">
                                <div class="form-group">
                                    <label for="empleado_inicio">No.Empleado - Inicio:</label>
                                      <select class="form-control select2bs4" style="width: 100%; text-align: left" id="empleado_inicio" name="empleado_inicio" required>
                                        <option value="">Selecciona</option>
                                        <?php foreach ($filasopr as $opr): //llenar las opciones del primer select ?>
                                       <option value="<?= $opr['noempleado'] ?>"><?= $opr['noempleado']. ' - ' .$opr['nombre_empleado'] ?></option>  
                                       <?php endforeach; ?>
                                     </select> 
                                </div>
                            </div>
                            <div class="col-4">
                                 <div class="form-group">
                                    ...
                                    <select class="form-control select2bs4" style="width: 100%; text-align: left" id="empleado_fin" name="empleado_fin" required>
                                        <option value="">Selecciona</option>
                                        <?php foreach ($filasopr as $opr): //llenar las opciones del primer select ?>
                                       <option value="<?= $opr['noempleado'] ?>"><?= $opr['noempleado']. ' - ' .$opr['nombre_empleado'] ?></option>  
                                       <?php endforeach; ?>
...
<script>
    document.getElementById('filtroReporte').addEventListener('submit', function(event) {
        var empleadoInicio = parseInt(document.getElementById('empleado_inicio').value);
        var empleadoFin = parseInt(document.getElementById('empleado_fin').value);

        // Validar que empleadoFin no sea menor que empleadoInicio
        if (empleadoFin < empleadoInicio) {
            event.preventDefault(); // Evita que el formulario se envíe
            
            // Mostrar alerta de SweetAlert
            Swal.fire({
                icon: 'error',
                title: 'Error en el rango de empleados',
                text: 'El número de empleado fin no puede ser menor que el número de empleado inicio.',
            });
        }
    });
</script>
