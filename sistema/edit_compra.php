...
session_start();
  ...
  {
    header('Location: compras23.php');
    mysqli_close($conection);
  }
  $idsol = $_REQUEST['id'];

  $sqlact= mysqli_query($conection,"SELECT * FROM compras WHERE no_compra=$idsol and estatus = 1");
  mysqli_close($conection);
  ...
  if($result_sqlact == 0){
    ...
    while ($data = mysqli_fetch_array($sqlact)){
      ...
      $nofactprov    = $data['nofactura_prov'];
      
    }
  }

    include "../conexion.php";
  $sqlbs= mysqli_query($conection,"SELECT nombre FROM proveedores WHERE id = $proveedor");
  mysqli_close($conection);
  ...
  $sqlrq   = "select id, no_requisicion from requisicion_compra where estatus = 2";
  $queryrq = mysqli_query($conection, $sqlrq);
  $filasrq = mysqli_fetch_all($queryrq, MYSQLI_ASSOC); 
  ...
  <div class="content-wrapper">
    ...
    <center>
       ...
       <?php
           /*         
          include "../conexion.php";
          $query_folio = mysqli_query($conection,"SELECT * FROM folios where serie = 'CO'");
          $result_folio = mysqli_num_rows($query_folio);

          $folioe = mysqli_fetch_array($query_folio);
          $nuevofolio=$folioe["folio"]+1; 

          $query_upfolio = mysqli_query($conection,"UPDATE folios SET folio= folio + 1 where serie = 'CO'");
     ...
     <div class="card card-secondary">
              <div class="card-header">
                <h3 class="card-title">Edicion de Compra</h3>
              </div>
              <div class="card-body">
              ...
              <div class="form-group row">
                  ...
                  <input style=" font-size: 14px;" type="hidden" class="form-control" id="inputId" name="inputId" value="<?php echo $id;?>">

                  <div class="form-group row" style="text-align:left;">
                    <label for="inputEmail3" class="col-sm-2 col-form-label">No. Orden Compra</label>
                    <div class="col-sm-4">
                      <select style=" font-size: 14px;"  class="form-control" style="width: 100%; text-align: left" id="inputNoorden" name="inputNoorden">
                       <option value="<?= $noorden ?>"><?= $noorden ?></option>
                       <?php foreach ($filasoc as $opoc): //llenar las opciones del primer select ?>
                       <option value="<?= $opoc['no_orden'] ?>"><?= $opoc['no_orden'] ?></option>  
                       <?php endforeach; ?>
                    </select>
                    </div>
                    ...
                    <div class="col-sm-4">
                      <input style="text-align: right; font-weight: bold; color: #F05B0E"  type="text" class="form-control" id="inputFolio" name="inputFolio" value="<?php echo $nocompra; ?>" readonly>
                    </div>
                  </div>
                  
                  <div class="form-group row" style="text-align:left;">
                     <label for="inputEmail3" class="col-sm-2 col-form-label">No. Requisición</label>
                    <div class="col-sm-4">
                      <select style=" font-size: 14px;"  class="form-control" style="width: 100%; text-align: left" id="inputNoreq" name="inputNoreq">
                       <option value="<?= $noreq ?>"><?= $noreq ?></option>
                       <?php foreach ($filasrq as $oprq): //llenar las opciones del primer select ?>
                       <option value="<?= $oprq['no_requisicion'] ?>"><?= $oprq['no_requisicion'] ?></option>  
                       <?php endforeach; ?>
                    </select>
                    </div> 
                    <label for="inputEmail3" class="col-sm-2 col-form-label">Fecha</label>
                    <div class="col-sm-4">
                      <input style=" font-size: 14px;" type="date" class="form-control" id="inputFecha" name="inputFecha" value="<?php echo $fecha;?>">
                    </div>
                  </div>
                   ...
                   <div class="form-group row" style="text-align:left;">
                    ...
                    <div class="col-sm-4">
                      <select style=" font-size: 14px;" class="form-control" style="width: 100%; text-align: left" id="inputProveedor" name="inputProveedor">
                       <option value="<?= $proveedor ?>"><?= $name_prov ?></option>
                    ...
                    <div class="col-sm-4">
                      <input style=" font-size: 14px;" type="text" class="form-control" id="inputContacto" name="inputContacto" value="<?= $contacto ?>">
                    </div>
                  </div>
                  ...
                  <div class="form-group row" style="text-align:left;">
                    ...
                    <div class="col-sm-4">
                      <input style=" font-size: 14px;" type="text" class="form-control" id="inputTelefono" name="inputTelefono"  value="<?= $telefono ?>">
                    </div>
                    <label for="inputEmail3" class="col-sm-2 col-form-label">Correo</label>
                    <div class="col-sm-4">
                      <input style=" font-size: 14px;" type="text" class="form-control" id="inputCorreo" name="inputCorreo"  value="<?= $correo ?>">
                    </div>
                  </div>
                  ...
                  <div class="form-group row" style="text-align:left;">
                    ...
                    <div class="col-sm-4">
                      <select style=" font-size: 14px;" name="inputUsocfdi" id="inputUsocfdi" required class="form-control custom-select" >
                            <option  value="<?= $uso_cfdi ?>"><?= $uso_cfdi ?></option>
                          ...
                          <div class="col-sm-4">
                          <select style=" font-size: 14px;" name="inputAreasolicita" id="inputAreasolicita" required class="form-control custom-select" >
                              <option  value="<?= $area_solicita ?>"><?= $area_solicita ?></option>
                  ...
                  <div class="form-group row" style="text-align:left;">
                    <label for="inputEmail3" class="col-sm-4 col-form-label">No. Factura del Proveedor</label>
                    <div class="col-sm-8"> 
                      <input type="text" class="form-control" id="inputNofacprov" name="inputNofacprov"  value="<?= $nofactprov ?>">
                    </div>
                  </div>
                 ...
                 <div class="form-group row" >
                   ...
                   <div class="col-sm-12" >
                          ...
                          <thead>
                            <tr>
                               ...
                               <th style="width:10%; background-color:#e9ecef; text-align: center;" >Almacen</th>
                               <th style="width:22%; background-color:#e9ecef; text-align: center;">Descripción</th>
                  ...
                  <div class="form-group row" style="text-align:left;">
                    ...
                    <div class="col-sm-10">
                      <textarea class="form-control" id="inputNotas" name="inputNotas" rows="1"><?= $notas ?></textarea>
                    </div>
                  </div>
     ...
     .then(resultado => {
        if (resultado.value) {
           ...
           var norecibo  = $('#inputFolio').val();
            var action = 'procesarSalirEditcompra';
...
<script>
   $('#guardar_tipoactividad').click(function(e){
       ...
       var idco        = $('#inputId').val();
       var folio       = $('#inputFolio').val();
       var noorden     = $('#inputNoorden').val(); 
       var noreq       = $('#inputNoreq').val();      
       ...
       var solicita    = $('#inputAreasolicita').val();
       var nofactura   = $('#inputNofacprov').val();
       var notas       = $('#inputNotas').val();
     
       var action       = 'AlmacenaEditCompra';

        $.ajax({
                    url: 'includes/ajax.php',
                    type: "POST",
                    async : true,
                    data: {action:action, idco:idco, folio:folio, noorden:noorden, noreq:noreq, fecha:fecha, proveedor:proveedor, contacto:contacto, telefono:telefono, correo:correo, forma_pago:forma_pago, metodo_pago:metodo_pago, uso_cfdi:uso_cfdi, solicita:solicita, nofactura:nofactura, notas:notas},
                    ...
                    success: function(response)
                    {
                      if(response != 'error')
                        ...
                        $mensaje=(info.mensaje);
                         ...
                         .fire({
                          title: "Exito!",
                          text: "COMPRA ALMACENADA CORRECTAMENTE",
                          icon: 'success',
      ...
      <div class="modal-body">
        ...
        <div class="col-md-12">
         ...
         <div class="form-group row" hidden>
           ...
           <div class="col-sm-9">
            <input type="text" class="form-control" id="inputfoliodet" name="inputfoliodet" value="<?php echo $nocompra; ?>" readonly>
           </div>
        </div> 
...
<script>
   $('#actualizaclientes').click(function(e){
        ...
        var impisr      = $('#inputImpuestoisr').val();
        var impieps     = $('#inputImpuestoieps').val();
        var importe     = $('#inputImporte').val();
        

       var action       = 'AddDetalleEditCompra';

        $.ajax({
                    url: 'includes/ajax.php',
                    type: "POST",
                    async : true,
                    data: {action:action, folio:folio, codigo:codigo, almacen:almacen, descripcion:descripcion, umedida:umedida, marca:marca, cantidad:cantidad, precio:precio, impuesto:impuesto, impisr:impisr, impieps:impieps, importe:importe},
...
<script>
    $('#inputNoorden').change(function(){
          //$('#disco_sel').html($(this).val() + ' - ' + $('#discos option:selected').text());
           ...
           var action   = 'addDetalleEditOcompra';
...
<script> 
  ...
  $('#modalEditCotizacion').on('show.bs.modal', function(e) {    
     ...
     var imp_isr    = $(e.relatedTarget).data().impisr;
     var imp_ieps   = $(e.relatedTarget).data().impieps;
     var importe   = $(e.relatedTarget).data().importe;
      ...
      $(e.currentTarget).find('#txt_impuestoisr').val(imp_isr);
      $(e.currentTarget).find('#txt_impuestoieps').val(imp_ieps);
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-md-12">
          ...
          <div class="form-group row">
           <label for="inputName2" class="col-sm-2 col-form-label" style="text-align: left; color:#737FA7; font-size: 14px;">Codigo:</label>
           <div class="col-sm-3">
            <select class="form-control" style="width: 100%; font-size: 14px" id="txt_codigo" name="txt_codigo">
                <option value="">- Seleccione -</option>
                <?php foreach ($filasprod as $oppd): //llenar las opciones del primer select ?>
                <option value="<?= $oppd['codigo'] ?>"><?= $oppd['codigo'] ?></option>  
                <?php endforeach; ?>
            ...
            <label for="inputName2" class="col-sm-2 col-form-label" style="text-align: left; color:#737FA7; font-size: 14px;">Descripción:</label>
           <div class="col-sm-5">
            <select class="form-control" style="width: 100%; text-align: left; font-size:12px" id="txt_descripcion" name="txt_descripcion">
                <option value="">- Seleccione -</option>
                <?php foreach ($filasprodnm as $opnm): //llenar las opciones del primer select ?>
                <option value="<?= $opnm['descripcion'] ?>"><?= $opnm['descripcion'] ?></option>  
                <?php endforeach; ?>
        ...
        <div class="form-group row">
         ...
         <div class="form-group row" >
           <label for="inputName2" class="col-sm-2 col-form-label" style="text-align: left; color:#737FA7;">IVA %:</label>
           <div class="col-sm-10">
...
<script>
   $('#actualiza_paradar').click(function(e){
        ...
        var action       = 'ActualizaMovEditcompra';
    
        $.ajax({
                    url: 'includes/ajax.php',
                    type: "POST",
                    async : true,
                    data: {action:action, detfolio:detfolio, detid:detid, detcodigo:detcodigo, detalmacen:detalmacen, detdescripc:detdescripc, detumedida:detumedida, detmarca:detmarca, detcantidad:detcantidad, detprecio:detprecio, detimpuesto:detimpuesto, detimpisr:detimpisr, detimpieps:detimpieps, detimporte:detimporte},
...
<script type="text/javascript">
  ...
  <script type="text/javascript">
    $(document).ready(function(){
        var c_noorden = '<?php echo $nocompra; ?>';
        //alert(c_noorden);
        searchForDetalleditCompra(c_noorden);
     });
  </script>

 <script>
 function searchForDetalleditCompra(c_noorden){
    var action = 'searchForDetalleditCompra';
...
<script>
    $('#inputNoreq').change(function(){
          //$('#disco_sel').html($(this).val() + ' - ' + $('#discos option:selected').text());
           var reqno  = $(this).val();
           var afolio = $('#inputFolio').val();
           var action   = 'addDetalleEditReq';

                $.ajax({
                    url: 'includes/ajax.php',
                    type: "POST",
                    async : true,
                    data: {action:action, reqno:reqno, afolio:afolio},
