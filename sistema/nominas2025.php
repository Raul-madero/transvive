		...
		const load_data = (semana, anio) => {
			...
			table = $('#example1').DataTable({
				...
				columns: [
					...
					{
						data: null,
						 render: (data) => formatoMoneda((parseFloat(data.sueldo_bruto) - parseFloat(data.nomina_fiscal)) + parseFloat(deposito_fiscal) + parseFloat(data.bono_semanal) + parseFloat(data.bono_supervisor) + parseFloat(data.bono_categoria) + parseFloat(data.apoyo_mes) + parseFloat(data.pago_vacaciones) + parseFloat(data.prima_vacacional) - parseFloat(data.deducciones) - parseFloat(data.caja_ahorro))
					}
				],
				language: {
