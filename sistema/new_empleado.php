...
$sqlnoempleado = "SELECT MAX(noempleado) AS noempleado FROM empleados";
$querynoempleado = mysqli_query($conection, $sqlnoempleado);
$maxno = mysqli_fetch_assoc($querynoempleado);
$noempleado = intval($maxno['noempleado']) + 1;
...
<body class="hold-transition layout-top-nav">
										...
										<div class="post clearfix">
											<form class="form-horizontal">
												...
												<div class="form-group row">
													...
													<div class="col-sm-10">
														<input type="number" class="form-control text-orange text-bold" id="inputNoempleado" name="inputNoempleado" value="<?php echo $noempleado ?>" readonly >
													</div>
												</div>
												<div class="form-group row">
												...
												<div class="form-group row">
													...
													<div class="col-sm-10">
														<input type="text" class="form-control" id="inputCorreo" name="inputCorreo" placeholder="Correo Electronico">
													</div>
												</div>
												<div class="form-group row">
												...
												<div class="border border-4 border-warning rounded-lg mb-4">
													...
													<div class="form-group row">
														...
														<div class="col-sm-5">
															<input type="text" class="form-control" id="inputNombreemergencia" name="inputNombreemergencia" placeholder="Nombre Contacto de Emergencia">
														</div>
														<label for="inputTelefonoemergencia" class="col-2 col-form-label text-left">Telefono:</label>
														<div class="col-3">
															<input type="text" class="form-control" id="inputTelefonoemergencia" name="inputTelefonoemergencia" placeholder="Telefono de Emergencia">
														</div>
													</div>
													<div class="form-group row">
														<label for="inputParentesco" class="col-4">Parentesco:</label>
														<div class="col-5">
															<input type="text" class="form-control" id="inputParentesco" name="inputParentesco" placeholder="Parentesco">
														</div>
													</div>
												</div>
											</form>
										...
										<div class="post clearfix">
											<form class="form-horizontal">
												<div class="form-group row">
													...
													<div class="form-group row">
														<label for="inputSueldoCamion" class="col-sm-3 col-form-label" style="text-align: left;">Sueldo Camion:</label>
														<div class="col-sm-3">
															<input type="number" class="form-control" id="inputSueldoCamion" name="inputSueldoCamion" step="0.01" value="180">
														</div>
						
														<label for="inputSueldoCamioneta" class="col-sm-3 col-form-label" style="text-align: left;">Sueldo Camioneta:</label>
														<div class="col-sm-3">
															<input type="number" class="form-control" id="inputSueldoCamioneta" name="inputSueldoCamioneta" step="0.01" value="120">
														</div>
													</div>
													...
													<div class="form-group row">
														<label for="inputAuto" class="col-sm-3 col-form-label" style="text-align: left;">Sueldo Automovil:</label>
														<div class="col-sm-3">
															<input type="number" class="form-control" id="inputAuto" name="inputAuto" step="0.01" value="120">
														</div>
														<label for="inputSprinter" class="col-sm-3 col-form-label" style="text-align: left;">Sueldo Sprinter:</label>
														<div class="col-sm-3">
															<input type="number" class="form-control" id="inputSprinter" name="inputSprinter" step="0.01" value="160">
														</div>
													</div>
												</form>
											</div>
										...
										<div class="tab-pane" id="settingss">
											<div class="post clearfix">
												<form class="form-horizontal">
													...
													<div class="form-group row">
														<label for="inputBonoSupervisor" class="col-sm-3 col-form-label" style="text-align: left;">Bono Supervisor:</label>
														<div class="col-sm-3">
															<input type="number" class="form-control" id="inputBonoSupervisor" name="inputBonoSupervisor" step="0.01" value="0">
														</div>
	...
	<script>
   		$('#guardar_cliente').click(function(e){
			...
			// Variables datos Operativos
			var tipoLic     = $('#inputTipolic').val();
			var nolicencia   = $('#inputNolicencia').val();
			var fvencimiento = $('#inputVencelicencia').val();
			var supervisor   = $('#inputSupervisor').val();

			// Variables datos Nomina
			var tipoContrato = $('#inputTipoContrato').val();
			var fechaContrato    = $('#inputInicoc').val();
			var venceContrato  = $('#inputVence').val();
			var imss         = $('#inputImss').val();
			var fechaAltaImss = $('#inputDatealtaimss').val();
			var noss         = $('#inputNss').val();
			var salarioDiario      = $('#inputSalariodia').val();
			var salarioPromedio   = $('#inputSalariodiario').val();
			var sueldoBase   = $('#inputSueldobase').val();
			var sueldoCamion       = $('#inputSueldoCamion').val();
			var sueldoCamioneta     = $('#inputSueldoCamioneta').val();
			var sueldoAuto   = $('#inputAuto').val();
			var sueldoSprinter     = $('#inputSprinter').val();
			
			// Variables datos pago Nomina
			var tipoNomina  = $('#inputTiponomina').val();
			var bonoSupervisor        = $('#inputBonoSupervisor').val();
			var cajaAhorro        = $('#inputCajaAhorro').val();
        	...
        	$.ajax({
				...
				dataType: 'json',
				data: {
					action,	noempleado, name, paterno, materno,	sexo, fechanac, edad, edocivil, domicilio, estudios, cargo, telefono, correo, rfc, curp, nombreEmergencia, telefonoEmergencia, parentesco,
					tipoLic,nolicencia, fvencimiento, supervisor,
					tipoContrato, fechaContrato, venceContrato, imss, fechaAltaImss, noss, salarioDiario, salarioPromedio, sueldoBase, sueldoCamion, sueldoCamioneta, sueldoAuto, sueldoSprinter,
					tipoNomina, bonoSupervisor, cajaAhorro
				},
                success: function (resp) {
					...
					if (resp && resp.ok === true) {
						...
						icon: 'error',
						title: 'Oops...',
						text: (resp && resp.error) ? resp.error : 'Ocurrió un error inesperado'
						});
					}
					},
					error: function (xhr) {
					console.error('AJAX error:', xhr.responseText);
					Swal.fire({ icon: 'error', title: 'Error de red/servidor', text: 'No se pudo completar la operación.' });
					}
            });
    	});
    </script>  
	...
	<script>
		$(function () {
			const $ini   = $('#inputInicoc');
			const $tipo  = $('#inputTipoContrato');
			const $vence = $('#inputVence');

			// Utilidad: formatea Date -> 'YYYY-MM-DD'
			...
			// Utilidad: parsea 'YYYY-MM-DD' a Date local sin problemas de TZ
			...
			function recalcularVencimiento() {
				const tipo = $tipo.val();
				const iniStr = $ini.val();
				if (tipo !== 'Eventual' || !iniStr) {
				$vence.val('');
				return;
				}

				const inicio = parseYMD(iniStr);
				if (!inicio || isNaN(inicio)) {
				$vence.val('');
				return;
				}

				// Opción A: sumar 30 días
				const fin = new Date(inicio);
				fin.setDate(fin.getDate() + 30);

				// --- Opción B (1 mes calendario): descomenta estas dos líneas y comenta las dos anteriores ---
				// const fin = new Date(inicio);
				// fin.setMonth(fin.getMonth() + 1);

				$vence.val(toISODate(fin));
			}

			// Recalcula cuando cambia la fecha de inicio o el tipo de contrato
			$ini.on('change', recalcularVencimiento);
			$tipo.on('change', recalcularVencimiento);
		});
	</script>
<!--
<script>
