...
$sql = "SELECT id as Id, date_format(fecha, '%d/%m/%Y') as Fcha, cliente, operador, unidad, num_unidad, personas, hora_inicio, time_format(hora_inicio, '%H:%i') as Hinicio, direccion, hora_fin, time_format(hora_fin, '%H:%i') as Hfin, destino, estatus, if(estatus = 1,'Activo',if(estatus = 2, 'Realizado', if(estatus=3 ,'Cancelado',if(estatus=4,'Iniciado',if(estatus=5,'Terminado',''))))) as Status, semana, hora_inicio, hora_fin, id_supervisor, jefe_operaciones  FROM registro_viajes where tipo_viaje <> 'Especial' and usuario_id = $idUser and fecha between '$fcha1' and '$fcha2' and estatus = 1 or estatus = 4 or estatus = 5   ";
}
