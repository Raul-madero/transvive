...
if (isset($_POST['semana'], $_POST['anio']) && !empty($_POST['semana']) && !empty($_POST['anio'])) {
    ...
    $total_nomina = $conection->query("SELECT SUM((sueldo_bruto - nomina_fiscal) + bono_semanal + bono_supervisor + bono_categoria + apoyo_mes + pago_vacaciones + prima_vacacional + sueldo_adicional - deducciones - caja_ahorro + deposito_fiscal) AS total_nomina FROM nomina_temp_2025")->fetch_assoc();
    ...
    $total_efectivo = $conection->query("SELECT SUM((sueldo_bruto - nomina_fiscal) + bono_semanal + bono_supervisor + bono_categoria + apoyo_mes + pago_vacaciones + prima_vacacional +sueldo_adicional - deducciones - caja_ahorro - deduccion_fiscal) AS total_efectivo FROM nomina_temp_2025")->fetch_assoc();
