    ...
    SUM(CASE 
        WHEN e.cargo = 'OPERADOR' THEN
            CASE 
                ...
                ELSE
                    CASE
                        WHEN LOWER(rv.unidad_ejecuta) REGEXP '\\bcamion\\b' AND IFNULL(r.sueldo_camion, 0) > 0 THEN (r.sueldo_camion, e.sueldo_camion) * rv.valor_vuelta
                        ...
                        WHEN LOWER(rv.unidad_ejecuta) REGEXP '\\bsprinter\\b'AND IFNULL(r.sueldo_sprinter, 0) THEN GREATEST(r.sueldo_sprinter, e.sueldo_sprinter) * rv.valor_vuelta
