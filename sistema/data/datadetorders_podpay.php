...
if($_REQUEST['action'] == 'fetch_users'){
    ...
    $columns = ' p.id, p.fecha, p.serie, p.folio, p.proveedor, p.documento, p.importe_pago, p.estatus, pv.nombre';
    $table = ' pagos_proveedor p inner join proveedores pv ON p.proveedor = pv.id ' ;
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        $nestedData['nocompra'] = $row["folio"];
        //$nestedData['noorden'] = $row['no_orden'];
        $nestedData['razonsoc'] = $row["nombre"];

        //$nestedData['area'] = $row["area_solicitante"];
        $nestedData['subtotal'] = $row["importe_pago"];
      
        $nestedData['impuesto'] = $row["importe_pago"];
        $nestedData['importetotal'] = $row["importe_pago"];
