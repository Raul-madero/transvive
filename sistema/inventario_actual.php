...
session_start();
  ...
  $sqlrefac   = "select * from refacciones where umedida <> 'Servicio' and estatus = 1 order by descripcion";
  $queryrefac = mysqli_query($conection, $sqlrefac);
  $filasrefac = mysqli_fetch_all($queryrefac, MYSQLI_ASSOC); 

  $sqlalmc    = "select * from almacenes where estatus = 1";
  $queryalmc  = mysqli_query($conection, $sqlalmc);
  $filasalmc  = mysqli_fetch_all($queryalmc, MYSQLI_ASSOC); 

  $sqlfac01   = "select descripcion from refacciones where umedida <> 'Servicio' and estatus = 1 order by descripcion DESC LIMIT 1";
  $queryrefac = mysqli_query($conection, $sqlrefac);
  $filasrefac = mysqli_fetch_all($queryrefac, MYSQLI_ASSOC); 

  //*include "../conexion.php";
  $sqlbs= mysqli_query($conection,"select descripcion, codigo from refacciones where umedida <> 'Servicio' and estatus = 1 order by descripcion ASC LIMIT 1");
  mysqli_close($conection);
  $result_sqlbs = mysqli_num_rows($sqlbs);

  
    while ($data2 = mysqli_fetch_array($sqlbs)){
      $primer_prod    = $data2['descripcion'];
      $codprimer_prod = $data2['codigo'];
   ...
   include "../conexion.php";
  $sqlbsut= mysqli_query($conection,"select descripcion, codigo from refacciones where umedida <> 'Servicio' and estatus = 1 order by descripcion DESC LIMIT 1");
  mysqli_close($conection);
  $result_sqlbsut = mysqli_num_rows($sqlbsut);

  
    while ($data3 = mysqli_fetch_array($sqlbsut)){
      $ultimo_prod    = $data3['descripcion'];
      $codultimo_prod = $data3['codigo'];
  ...
  <div class="content-wrapper">
   <section class="content">
        <div class="container-fluid">
            <h2 class="text-center">Reporte de Inventario Actual</h2>
             <form action="factura/inventario_actualpdf.php" method="post" target="_blank">
                <div class="row">
                    <div class="col-md-10 offset-md-1">
                        <div class="row">
                            <div class="col-3">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control" id="fechaini" name="fechaini" onchange="myFunctionDate()" value="<?= $inicio ?>">
                                </div>
                            </div>
                            <div class="col-3">
                                <div class="form-group">
                                    <label>Fecha Final:</label>
                                    <input type="date" class="form-control" id="fechafin" name="fechafin" onchange="myFunctionDateTwo()" value="<?= $fin ?>">
                                </div>
                            </div>
                            <!--
                            <div class="col-3">
                          ...
                          <div class="col-12">
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2bs4" id="productoini" name="productoini" >
                                    <option value="<?= $codprimer_prod ?>"><?= $primer_prod ?></option>
                                     <?php foreach ($filasrefac as $oprf): //llenar las opciones del primer select ?>
                                    <option value="<?= $oprf['codigo'] ?>"><?= $oprf['descripcion'] ?></option>  
                                    <?php endforeach; ?>
                                    </select>
                            ...
                            <div class="col-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2bs4" id="productofin" name="productofin" >
                                    <option value="<?= $codultimo_prod ?>"><?= $ultimo_prod ?>"</option>
                                     <?php foreach ($filasrefac as $oprf): //llenar las opciones del primer select ?>
                                    <option value="<?= $oprf['codigo'] ?>"><?= $oprf['descripcion'] ?></option>  
                                    <?php endforeach; ?>
                                    </select>
                      ...
                      -->
                      <div class="col-2">
                      <button name="buscar" type="submit" class="btn btn-danger pull-left"><i class="fa fa-file-pdf"></i> Aceptar</button>
                      </div>
                    <div class="col-2">
