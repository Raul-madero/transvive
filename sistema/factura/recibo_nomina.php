...
function generarReciboSemanal($pdf, $conection, $semanaTexto, $anio) {
    ...
    $fechaInicial = $fechaInicio->format('Y-m-d');
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $stmtVueltas->bind_param("sss", $row['nombre'], $fechaInicial, $fechaFinal);
        $stmtVueltas->execute();
        ...
        $pdf->Cell(60, 5,'Deposito Efectivo: $' . number_format($row['sueldo_bruto'] + $row['bono_semanal'] + $row['bono_supervisor'] + $row['bono_categoria'] + $row['sueldo_adicional'] + $row['apoyo_mes'] + $row['prima_vacacional'] + $row['pago_vacaciones'] - $row['deducciones'] - $row['caja_ahorro'] - $row['deduccion_fiscal'] - $row['deposito_fiscal'], 2), 0, 1, 'R');
        ...
        if ($resultVueltas->num_rows > 0) {
            while ($rowV = $resultVueltas->fetch_assoc()) {
                ...
                $pdf->Cell(30, 6, (number_format($rowV['valor_vuelta'], 2)) == 1.00 ? "Completa" : "Media", 1, 1, 'R');
            }
        } else {
