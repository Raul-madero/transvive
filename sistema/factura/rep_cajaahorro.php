...
header("Content-Disposition: attachment; filename= Rep. Caja de Ahorro por Periodos Semanal.xls");
...
$query_productos = mysqli_query($conection,"SELECT 
    ...
    dn.imss, 
    dn.estatus, 
    SUM(dn.caja) AS Caja_ahorro, 
    em.caja_ahorro AS caja_base,
    MIN(dn.no_semana) AS primer_movimiento
FROM 
    detalle_nomina dn
INNER JOIN 
    empleados em 
    ON dn.no_empleado = em.noempleado
WHERE 
    dn.no_semana BETWEEN '$fecha_finsem' AND '$fecha_fin2sem' 
    AND dn.yearpago = $fecha_ejerciciosem
GROUP BY 
    dn.no_empleado
ORDER BY 
    dn.no_empleado;");
...
<?php echo 'REPORTE DE CAJA DE AHORRO POR RANGO DE SEMANAS'. ' DE: '. $fecha_finsem. ' A LA: '. $fecha_fin2sem ; ?>
<table border="1">
    ...
    <th>SEMANA 1ER. DESCUENTO</th>
    <th>TOTAL AHORRADO</th>
    <th>NO. SEMANAS</th>
    <th style="background-color: #12a90b; color: white;">ACUMULADO SEMANAL</th>
    </tr>
    </thead>
<?php
 while ($row=mysqli_fetch_assoc($query_productos)) {
        if ($row['Caja_ahorro'] > 0) {
           $nodesemanas = $row['Caja_ahorro'] / $row['caja_base'] ;
           $semanadesc  = $row['primer_movimiento'];
        } else {
           $nodesemanas = 0 ;
           $semanadesc  = "";
        } 
      ...
      ?>
        <tr>
          ...
          <td><?php echo number_format($row['caja_base'],2); ?></td>
          <td><?php echo strtoupper($semanadesc); ?></td>
          <td><?php echo number_format($row['Caja_ahorro'],2); ?></td>
          <td><?php echo number_format($nodesemanas,0); ?></td>
          <td><?php echo number_format($row['Caja_ahorro'],2); ?></td>
        </tr> 
